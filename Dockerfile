...
RUN go get github.com/docker/swarm && cd /go/src/github.com/docker && rm -fr swarm && git clone https://github.com/denverdino/swarm.git && cd /go/src/github.com/docker/docker && sh project/make/.go-autogen && cd /go/src/github.com/docker/swarm && go build && go install
